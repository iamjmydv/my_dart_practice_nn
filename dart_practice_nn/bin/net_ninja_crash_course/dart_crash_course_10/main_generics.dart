//------------------------------------------------------------------------------
// G E N E R I C S  —  C O M P L E T E  G U I D E
//------------------------------------------------------------------------------
//
// This file combines ALL 7 samples into one runnable file.
...
// -----------------------------------------------------------------------
//
// NOTE: All class and function names are PREFIXED (S1, S2, ...) to avoid
// name collisions between samples. Each sample's standalone file uses
...
void main() {
  ...
  sample1Problem();
  sample2BasicGenericClass();
  sample3MultipleTypeParams();
  sample4GenericMethods();
  sample5BoundedGenerics();
  sample6GenericsWithInheritance();
  sample7RealWorldMenuCollection();
}

// =============================================================================
// =============================================================================
//
//  S A M P L E  1:  THE PROBLEM WITHOUT GENERICS — Why dynamic is risky
//
...
//  put a String where you expected an int.
...
void sample1Problem() {
  print('');
  print('##############################################');
  print('# SAMPLE 1: The Problem Without Generics     #');
  print('##############################################');

  // WITHOUT generics — uses dynamic, no type safety
  var unsafeBox = S1UnsafeBox('Hello');
  print('  UnsafeBox item: ${unsafeBox.item}'); // Hello
  print('  Type: ${unsafeBox.item.runtimeType}'); // String

  // This compiles fine, but could crash at runtime if you
  // tried to use .item as an int when it's actually a String:
  var anotherUnsafeBox = S1UnsafeBox(42);
  print('  Another UnsafeBox: ${anotherUnsafeBox.item}'); // 42

  // WITH generics — Dart knows the exact type
  var safeStringBox = S1SafeBox<String>('Hello');
  var safeIntBox = S1SafeBox<int>(42);
  print('  SafeBox<String>: ${safeStringBox.item}'); // Hello
  print('  SafeBox<int>: ${safeIntBox.item}'); // 42

  // This would cause a COMPILE-TIME error (caught before running!):
  // var badBox = S1SafeBox<int>('Hello');  // Error! String is not int
...
// =============================================================================
//
//  S A M P L E  2:  BASIC GENERIC CLASS — Type-safe reusable containers
//
...
void sample2BasicGenericClass() {
  print('##############################################');
  print('# SAMPLE 2: Basic Generic Class               #');
  print('##############################################');

  // Stack<T> — a last-in-first-out data structure
  var numberStack = S2Stack<int>();
  numberStack.push(10);
  numberStack.push(20);
  numberStack.push(30);
...
// Works with any type: S2Stack<int>, S2Stack<String>, etc.
class S2Stack<T> {
  final List<T> _items = [];
...
// =============================================================================
//
//  S A M P L E  3:  MULTIPLE TYPE PARAMETERS — When you need more than one type
//
...
void sample3MultipleTypeParams() {
  print('##############################################');
  print('# SAMPLE 3: Multiple Type Parameters          #');
  print('##############################################');

  // Pair<A, B> — holds two values of potentially different types
  var nameAndAge = S3Pair<String, int>('Alice', 30);
  ...
  var coordinates = S3Pair<double, double>(42.3, -71.1);
  print('  $coordinates'); // Pair(42.3, -71.1)

  // KeyValue<K, V> — like a Map entry
  var setting = S3KeyValue<String, bool>('darkMode', true);
  print('  $setting'); // darkMode: true

  var score = S3KeyValue<String, int>('Player1', 9500);
...
class S3KeyValue<K, V> {
  K key;
  V value;

  S3KeyValue(this.key, this.value);
...
// =============================================================================
//
//  S A M P L E  4:  GENERIC METHODS / FUNCTIONS — Generic logic without a class
//
...
void sample4GenericMethods() {
  print('##############################################');
  print('# SAMPLE 4: Generic Methods / Functions       #');
  print('##############################################');

  // Generic function: s4GetFirst<T> — returns the first item of any list
  print('  First int: ${s4GetFirst<int>([10, 20, 30])}'); // 10
  print('  First string: ${s4GetFirst<String>(['a', 'b', 'c'])}'); // a

  // Dart can INFER the type — you don't need to write <int>
  print('  Inferred: ${s4GetFirst([100, 200, 300])}'); // 100

  // Generic function: s4WrapInList<T> — wraps any single value in a list
  print('  Wrap int: ${s4WrapInList(42)}'); // [42]
  print('  Wrap string: ${s4WrapInList('hello')}'); // [hello]
  print('  Wrap bool: ${s4WrapInList(true)}'); // [true]

  // Generic function: s4SwapPair<A, B> — swaps the values in a Pair
  var original = S4Pair<String, int>('age', 25);
  var swapped = s4SwapPair(original);
...
// --- Sample 4: Function and class definitions ---

// Returns the first element of any typed list
T s4GetFirst<T>(List<T> items) {
  if (items.isEmpty) throw StateError('List is empty');
  return items.first;
}

// Wraps any single value into a List
List<T> s4WrapInList<T>(T item) {
  return [item];
}

// Swaps the two values in a Pair — note the return type flips from <A,B> to <B,A>
S4Pair<B, A> s4SwapPair<A, B>(S4Pair<A, B> pair) {
  return S4Pair<B, A>(pair.second, pair.first);
}

// Pair class for Sample 4 (needed for swapPair)
class S4Pair<A, B> {
  A first;
  B second;

  S4Pair(this.first, this.second);
...
// =============================================================================
//
//  S A M P L E  5:  BOUNDED GENERICS (extends) — Restrict what types are allowed
//
...
void sample5BoundedGenerics() {
  print('##############################################');
  print('# SAMPLE 5: Bounded Generics                  #');
  print('##############################################');

  // MathBox<T extends num> — only works with numbers (int, double)
  var intMath = S5MathBox<int>([3, 7, 1, 9, 4]);
  ...
  var doubleMath = S5MathBox<double>([2.5, 8.1, 3.7]);
  print('  Sum: ${doubleMath.sum()}'); // Sum: 14.3

  // This would NOT compile — String doesn't extend num:
  // var stringMath = S5MathBox<String>(['a', 'b']);  // Error!

  // SortedCollection<T extends Comparable> — items must be sortable
  var sortedNumbers = S5SortedCollection<int>([5, 2, 8, 1, 9, 3]);
  print('  Sorted ints: ${sortedNumbers.sorted()}'); // [1, 2, 3, 5, 8, 9]

  var sortedWords = S5SortedCollection<String>(['banana', 'apple', 'cherry']);
  print('  Sorted words: ${sortedWords.sorted()}'); // [apple, banana, cherry]
...
class S5MathBox<T extends num> {
  List<T> numbers;

  S5MathBox(this.numbers);
...
// SortedCollection<T extends Comparable> — items must be sortable
class S5SortedCollection<T extends Comparable> {
  List<T> items;

  S5SortedCollection(this.items);
...
// =============================================================================
//
//  S A M P L E  6:  GENERICS WITH INHERITANCE — Combine generics + class hierarchies
//
...
void sample6GenericsWithInheritance() {
  print('##############################################');
  print('# SAMPLE 6: Generics with Inheritance         #');
  print('##############################################');

  var userRepo = S6LoggedRepository<String>('UserRepo');
  userRepo.add('Alice');
  userRepo.add('Bob');
  userRepo.add('Carol');
  // Prints:
  ...
  var numberRepo = S6LoggedRepository<int>('NumberRepo');
  numberRepo.add(100);
  numberRepo.add(200);
  // Prints:
...
class S6LoggedRepository<T> extends S6Repository<T> {
  String repoName;

  S6LoggedRepository(this.repoName);
...
// =============================================================================
//
//  S A M P L E  7:  REAL-WORLD EXAMPLE — Menu Collection
//
...
void sample7RealWorldMenuCollection() {
  print('##############################################');
  print('# SAMPLE 7: Real-World Menu Collection        #');
  print('##############################################');

  var noodle = S7MenuItem('Veg Noodles', 9.99);
  var pizza = S7Pizza(['pepperoni', 'mushrooms'], 'Veg Pizza', 15.99);
  var roast = S7MenuItem('Veggie Roast Dinner', 12.49);
  var kebab = S7MenuItem('Plant Kebab', 7.49);
  ...
  var menu = S7Collection<S7MenuItem>(
    'Food Menu',
    [noodle, pizza, roast, kebab],
  );
  ...
  var luckyNumbers = S7Collection<int>('Lucky Numbers', [7, 13, 21, 42]);
  print('  ${luckyNumbers.name}: ${luckyNumbers.data}');
  print('  Random lucky number: ${luckyNumbers.randomItem()}');

  var greetings = S7Collection<String>(
    'Greetings',
    ['Hello', 'Hi', 'Hey', 'Howdy'],
  );
...
// --- Sample 7: Class definitions ---

class S7MenuItem {
  String title;
  double price;

  S7MenuItem(this.title, this.price);
...
class S7Pizza extends S7MenuItem {
  List<String> toppings;

  S7Pizza(this.toppings, super.title, super.price);
...
class S7Collection<T> {
  String name;
  List<T> data;

  S7Collection(this.name, this.data);
...
// =============================================================================
// =============================================================================
//  NOTES: Generics — Complete Summary of ALL 7 Samples
// =============================================================================
//
//  WHAT ARE GENERICS?
//  ------------------
